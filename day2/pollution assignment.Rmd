...
#reads all files for the given ids in the given folder
readAllFiles <- function(directory, id){
    csvData <- c()
    for (i in id) {
        filename <- paste0(directory, "/", formatC(i, width=3, flag="0"), ".csv")
        
        csvData <- rbind(csvData,read.csv(filename))
    }
    csvData
}
...
pmean <- function(directory, pollutant, id = 1:332) {
    ...
    csvData <- readAllFiles(directory, id)
    
    pollutantData <- csvData[[pollutant]]
    
    mean(na.omit(pollutantData))
}
...
complete <- function(directory, id = 1:332) {
    ...
    # sum up all data for each station
    na.omit(csvData) %>% group_by(ID) %>% tally(name="nobs")
}
```
...
corr <- function(directory, threshold = 0) {
    ...
    for (fileName in fileNames) {
        ...
        if(tally(csvData)>threshold){
        
            corrs <- c(corrs, cor(csvData$sulfate, csvData$nitrate))
        }
    }    
    corrs
}
